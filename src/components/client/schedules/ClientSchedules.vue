...
const scheduleStats = computed((): ScheduleStatsType => {
  ...
  const upcoming = userSchedules.value.filter(
    (schedule) => {
      const { date, month, year } = dateMonthYearRetrieve(
        formatScheduleTime(schedule)
      );
  ...
  const past = userSchedules.value.filter(
    (schedule) => {
      const { date, month, year } = dateMonthYearRetrieve(
        formatScheduleTime(schedule)
      );
      return (
        moment(
          ...
          `${dateTimeToday.getFullYear()}-${dateTimeToday.getMonth() + 1}-${dateTimeToday.getDate()}`
        )
      );
    }
  ).length;
  const thisWeek = userSchedules.value.filter(
    (schedule) => {
      const { date, month, year } = dateMonthYearRetrieve(
        formatScheduleTime(schedule)
      );
  ...
  const thisMonth = userSchedules.value.filter(
    (schedule) => {
      const { date, month, year } = dateMonthYearRetrieve(
        formatScheduleTime(schedule)
      );
  ...
  const thisYear = userSchedules.value.filter(
    (schedule) => {
      const { date, month, year } = dateMonthYearRetrieve(
        formatScheduleTime(schedule)
      );
