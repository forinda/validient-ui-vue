...
<template>
  <main class="flex-col py-10">
    ...
    <div class="bg-white w-full border">
      ...
      >
        <div
          v-for="key of Object.keys(adminStats)"
          class="h-full flex flex-col p-4 justify-between border rounded-lg bg-color8"
        >
          <div class="flex justify-between">
            <div class="flex flex-col gap-2">
              <h1 class="color-1 font-bold text-3xl">
                {{ adminStats[key as AppointmentStatsKeys].count.toString().padStart(2, "0") }}
              </h1>
              <p>{{ adminStats[key as AppointmentStatsKeys].title }}</p>
            </div>
            <div>
              <fa
                :icon="adminStats[key as AppointmentStatsKeys].icon!"
    ...
    <div v-if="linkedOrPersonalAppointments.length" class="grid grid-cols-[1fr_280px]">
      <div>
        <!-- component -->
      ...
      <div class="p-8 border rounded h-fit">
        ...
        <ul v-if="adminStats.today.items.length" class="flex flex-col gap-4">
          <li
            v-for="appointment in adminStats.today.items"
            :key="appointment.id"
