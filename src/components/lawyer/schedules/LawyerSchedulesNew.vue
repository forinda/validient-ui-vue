        ...
        <form @submit.prevent="handleSubmit" class="form-component border w-full">
            <div>
                <h1 class="text-2xl font-bold text-center">
                    New Schedule for
                    <span class="px-4 text-underline text-blue-500 border rounded">@{{ authStateUser?.username }}</span>
                </h1>
            </div>
            <div class="input-wrapper">
                <label for="name">Schedule title</label>
                <input required type="text" id="name" v-model="newSchedule.title" class="input-field"
                    placeholder="Case status name eg." />
            </div>
            <div class="input-wrapper">
                <label for="name">Select date and time</label>
                <input required type="datetime-local" id="name" v-model="date" class="input-field"
                    placeholder="Case status name eg." />
            </div>
            <div class="input-wrapper">
                <label for="description">Description</label>
                <textarea required type="text" id="description" v-model="newSchedule.description" class="input-field h-32"
