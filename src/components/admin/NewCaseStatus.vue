...
import { CaseStatusType } from "@/typings";
import { ref } from "vue";
...
const newCaseStatus = ref<Omit<CaseStatusType, "id">>({
  description: "",
  name: "",
});
const handleSubmit = async (e: Event) => {
  ...
  if (target.checkValidity()) {
    try {
      ...
      response.status === 201 && router.push({ name: "caseStatuses" });
    } catch (error) {
      console.log(error);
    }
  }
  ...
  e.preventDefault();
  console.log(newCaseStatus.value);
};
</script>
<template>
    ...
    <form @submit.prevent="handleSubmit" class="form-component">
      ...
      <div class="input-wrapper">
        ...
        <input
          ...
          type="text"
          id="description"
          v-model="newCaseStatus.description"
          class="input-field"
        />
      </div>
