...
onMounted(() => {
  ...
  return Promise.resolve(fetchUsers())
})
...
type userStatsKeys = 'users' | 'lawyers' | 'admins' | 'total' | 'male' | 'female'
type userStatsType = {
...
const computedUserStats = computed((): userStatsType => {
  const users = tableBody.value.filter((user) => filterUserRoles(user.roles.map(role => role.name.toLowerCase())) === 'user')
  const lawyers = tableBody.value.filter((user) => filterUserRoles(user.roles.map(role => role.name.toLowerCase())) === 'lawyer')
  const admins = tableBody.value.filter((user) => filterUserRoles(user.roles.map(role => role.name.toLowerCase())) === 'admin')
  const males = tableBody.value.filter(u => u.gender.toLowerCase() === 'male')
  const females = tableBody.value.filter(u => u.gender.toLowerCase() === 'female')
  const other = tableBody.value.filter(u => u.gender.toLowerCase() !== 'male' || u.gender.toLowerCase() !== 'female')
  const userStats = {
    users: {
      count: users.length,
...
<template>
  <main class="p-4 flex flex-col gap-4">
    ...
    <div class="bg-white border-b-color2 border-b p-4 rounded flex justify-between items-center">
      ...
      <div>
        <button class="bg-color1 text-white font-bold rounded-full px-4 py-1">
          create new user
        </button>
      </div>
    </div>
    ...
    <div class="bg-white border overflow-scroll no-scrollbar">
      <table class="table-auto w-full text-sm text-center text-gray-500 dark:text-gray-400 border p-4 m-4 rounded-sm">
