...
import { SignUpFormPropType } from "@/typings";
...
<template>
  <main class="flex justify-center items-center py-6 min-h-[60vh]">
    <form
      action=""
      class="w-full p-10 max-w-sm lg:max-w-xl bg-neutral-100 rounded-lg shadow-md border"
      ...
      <div v-if="currentStep === 'personal-info'" class="flex flex-col">
        ...
        <label class="flex flex-col gap-2 text-gray-700 text-xs font-bold mb-2">
          ...
          <p v-if="personalInfo$.firstName.$error">
            <span
              v-for="item in personalInfo$.firstName.$errors"
              :key="item.$uid"
        ...
        <label class="flex flex-col gap-2 text-gray-700 text-xs font-bold mb-2">
          ...
          <p v-if="personalInfo$.lastName.$error">
            <span
              v-for="item in personalInfo$.lastName.$errors"
              :key="item.$uid"
              class="text-red-500 text-sm"
            >
              {{ item.$message }}
        ...
        <label class="flex flex-col gap-2 text-gray-700 text-xs font-bold mb-2">
          ...
          <p v-if="personalInfo$.email.$error">
            <span
              v-for="item in personalInfo$.email.$errors"
              :key="item.$uid"
              class="text-red-500 text-sm"
            >
              {{ item.$message }}
        ...
        <div>
          <button
            class="w-full px-10 py-2 rounded-full text-color2"
            @click.prevent="handleNext('password')"
            :class="[
              personalInfo$.$error
      ...
      <div v-if="currentStep === 'password'" class="flex flex-col">
        <label class="flex flex-col gap-2 text-gray-700 text-xs font-bold mb-2">
          ...
          <p v-if="passwordInfo$.password.$error">
            <span
              v-for="item in passwordInfo$.password.$errors"
              :key="item.$uid"
        ...
        ><label class="flex flex-col gap-2 text-gray-700 text-xs font-bold mb-2">
          ...
          <p v-if="passwordInfo$.confirmPassword.$error">
            <span
              v-for="item in passwordInfo$.confirmPassword.$errors"
              :key="item.$uid"
              class="text-red-500 text-sm block"
            >
              {{ item.$message }}
        ...
        <div>
          <button
            :disabled="!validForm"
            class="w-full bg-color1 text-lg py-2 px-4 rounded-lg"
